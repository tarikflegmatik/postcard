...
const SponsorLogos = ({
  isEventSpecialLogos = false,
}: {
  isEventSpecialLogos?: boolean;
}) => {
  const logos = isEventSpecialLogos ? eventSpecialLogos : defaultLogos;
  return (
    <motion.div
      className="mt-12 flex h-fit w-full items-center justify-around gap-4 bg-white p-2 sm:p-5"
    ...
    >
      {logos.map((logo, index) => (
        <motion.div
          ...
          className={`${isEventSpecialLogos ? "h-10" : "h-8"} w-auto`}
        >
          <Image
